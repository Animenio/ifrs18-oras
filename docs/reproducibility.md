...
4. Validate the default JSON codebook with `python -m ifrs18_oras validate-codebook --codebook config/codebook_v0.1.1.json`; validate `config/codebook_v0.1.0.json` when reproducing earlier pilot runs.
5. Run `python -m ifrs18_oras score --input-dir data/raw --output-dir outputs/run_001 --codebook config/codebook_v0.1.1.json --preferred-format xhtml` for recommended EU ESEF core scoring, or use `--preferred-format all` for default mixed-package scoring.
